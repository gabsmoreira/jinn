...
// The engine registers TWO proc.onData listeners (the stream snapshot + our bg-agent
// scanner). This FakePty keeps the LAST one — the scanner — so the test can feed it
// the refusal marker directly.
interface FakePty {
  pid: number;
  _dataCb?: (d: string) => void;
  _exitCb?: (e: { exitCode: number; signal: number }) => void;
...
describe("InteractiveClaudeEngine — bg-agent resume guard (upstream snapshot model)", () => {
  ...
  it("detects the refusal in output, blocks respawn, and clear re-enables it", async () => {
    ...
    ptys[0]._exitCb!({ exitCode: 1, signal: 0 });
    await flush();
  ...
  it("trips the fast-exit backstop after repeated immediate exits", async () => {
    for (let i = 0; i < 3; i++) {
      engine.ensureIdleSpawn(SID, { engineSessionId: "e1", cols: 80, rows: 24 } as any);
      await flush();
      ptys[ptys.length - 1]._exitCb!({ exitCode: 1, signal: 0 });
      await flush();
    }
