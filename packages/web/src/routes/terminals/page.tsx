                    ...
                    {!collapsed.has(g.agent) && (
                      <>
                        {home ? (
                          <RailRow
                            s={home}
                            selected={home.id === selectedId}
                            onSelect={setSelectedId}
                            label="💬 Home chat"
                            pinned={pinned.has(home.id)}
                            renaming={renamingId === home.id}
                            canDelete={false}
                            onTogglePin={() => togglePin(home.id)}
                            onStartRename={() => setRenamingId(home.id)}
                            onCommitRename={(v) => handleRename(home.id, v)}
                            onFork={() => handleFork(home.id)}
                            onRequestDelete={() => {}}
                          />
                        ) : null}
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) setDeleteTarget(null) }}>
        <DialogContent showCloseButton={false} className="max-w-sm" onOpenAutoFocus={(e) => { e.preventDefault(); deleteButtonRef.current?.focus() }}>
          ...
          <DialogFooter>
            ...
            <Button ref={deleteButtonRef} variant="destructive" onClick={handleConfirmDelete}>
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
